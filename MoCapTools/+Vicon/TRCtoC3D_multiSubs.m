function c3dFileOut = TRCtoC3D_multiSubs(markerTable, c3dFileOrig, c3dFileOut)
...
%     end
    btkSetFirstFrame(c3dHandle,header(1));
    btkSetFrameNumber(c3dHandle,header(end)-header(1)+1); 
    ...
    splitfun = @(x) regexp(x,'_','split');
    reconfun = @(x) strcat(strjoin(x(1:end-1), '_'),':',x(end));
    labels_split = cellfun(splitfun,labels,'UniformOutput',false);
    labels_recon = cellfun(@(x) processLabel(x, reconfun), labels_split, 'UniformOutput', false);
    labels_recon = cellfun(@(x) char(x{1}),labels_recon,'UniformOutput',false);
    labels = labels_recon;
...
function output = processLabel(x, reconfun)
    if numel(x) > 1
        output = reconfun(x);
    else
        output = x;
