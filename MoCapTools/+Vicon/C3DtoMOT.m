...
%     time = (frames - 1)/samplingFreq;
    ...
    for fp_idx=1:length(fpList)
        ...
        if FilterFreq>0
            ...
            for ii=1:3
                fpValues.F(:,ii)=filtfilt(b,a,fpValues.F(:,ii));
                fpValues.M(:,ii)=filtfilt(b,a,fpValues.M(:,ii));
%                 fpValues.P(:,ii)=filtfilt(b,a,fpValues.P(:,ii)); % Filter CoP
            end
            fpValues.F(fpValues.F(:,3)<20,3)=0;
            fpValues.M(fpValues.F(:,3)<20,1:2)=0;
        end
...
%         H=getTransform(corners_0)';
        ...
        width = sqrt(sum((corners(:,1)-corners(:,2)).^2));
        height = sqrt(sum((corners(:,4)-corners(:,1)).^2));
        
        p_c_topLeft = [width/2, height/2, 0]';
        p_c_topRight = [-width/2, height/2, 0]';
        p_c_botRight = [-width/2, -height/2, 0]';
        p_c_botLeft = [width/2, -height/2, 0]';
        
        p_o_c = -origin;
        
        p_o_topLeft = p_o_c + p_c_topLeft;
        p_o_topRight = p_o_c + p_c_topRight;
        p_o_botRight = p_o_c + p_c_botRight;
        p_o_botLeft = p_o_c + p_c_botLeft;
        
        pattern = [p_o_topLeft, p_o_topRight, p_o_botRight, p_o_botLeft];
        
        H = getTransform2(corners, pattern)';
...
function H = getTransform2(corners, pattern)
% Compute the homogeneous transformation matrix between the corners tile
% frame to the pattern tile frame centered at the origin.
