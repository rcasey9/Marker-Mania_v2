function markerStruct = pickupMarkerDrops(markerSet,markerStruct,markerStructRef,clusters,gap_th,check_th,patternCheckth,direction,gap_len,iter,markerJumpThresholdSet,jumpThresholdList)
...
%         currentMarker = 'RSHO';
    ...
    %                     disp(['  Searching: ',currentMarker,' at frame: ',num2str(loc+startFrameOffset-gap)]);
        ...
        %                                     disp(['          Found at: ',cmp_marker])
                                            ...
                                            false_flag2 = 2;
                                            doubleCheckedNum2 = 0;
                                            %% Double Checking Using Other Cluster Markers Length
                                            for ee = 1:length(currentCluster)
                                                currentClusterMarker2 = currentCluster{ee}; % cluster marker that we're looking at
                                                if strcmp(currentClusterMarker2,currentClusterMarker);continue;end
                                                if ~isnan(markerStruct.(currentClusterMarker2).x(loc))
                                                    currentClusterMarkerReference2 = [markerStructRef.(currentClusterMarker2).x(1); markerStructRef.(currentClusterMarker2).y(1); markerStructRef.(currentClusterMarker2).z(1)];
                                                    currentMarkerReference = [markerStructRef.(currentMarker).x(1); markerStructRef.(currentMarker).y(1); markerStructRef.(currentMarker).z(1)];
                                                    cmpReferenceLength = (sum((currentMarkerReference-currentClusterMarkerReference2).^2)).^0.5;
                                                    currentClusterMarkerCoordinate2 = [markerStruct.(currentClusterMarker2).x(loc); markerStruct.(currentClusterMarker2).y(loc); markerStruct.(currentClusterMarker2).z(loc)];
                                                    currentcmpMarkerCoordinate = [markerStruct.(cmp_marker).x(loc); markerStruct.(cmp_marker).y(loc); markerStruct.(cmp_marker).z(loc)];
                                                    cmpCurrentLength = (sum((currentcmpMarkerCoordinate-currentClusterMarkerCoordinate2).^2)).^0.5;
    ...
    %                                                     disp('Double Checked!')
                                            ...
                                            %% Double Check Using Neight Frames Cluster Marker Length
                                            bw_gap = 3;
                                            fw_gap = 3;
                                            if loc-bw_gap < 1
                                                bw_gap = loc - 1;
                                            elseif loc+fw_gap > length(markerStruct.(markerStructname).Header)
                                                fw_gap = length(markerStruct.(markerStructname).Header) - loc;
                                            end
                                            for ee = 1:length(currentCluster)
                                                currentClusterMarker2 = currentCluster{ee}; % cluster marker that we're looking at
                                                if strcmp(currentClusterMarker2,currentClusterMarker);continue;end
                                                if ~isnan(markerStruct.(currentClusterMarker2).x(loc))
                                                    currentClusterMarkerCoordinate2 = [markerStruct.(currentClusterMarker2).x(loc-bw_gap:loc+fw_gap),markerStruct.(currentClusterMarker2).y(loc-bw_gap:loc+fw_gap),markerStruct.(currentClusterMarker2).z(loc-bw_gap:loc+fw_gap)];
                                                    currentMarkerCoordinate = [markerStruct.(currentMarker).x(loc-bw_gap:loc+fw_gap),markerStruct.(currentMarker).y(loc-bw_gap:loc+fw_gap),markerStruct.(currentMarker).z(loc-bw_gap:loc+fw_gap)];
                                                    currentMarkerCoordinate(bw_gap + 1,:) = [markerStruct.(cmp_marker).x(loc); markerStruct.(cmp_marker).y(loc); markerStruct.(cmp_marker).z(loc)];
                                                    LengthList2 = currentMarkerCoordinate-currentClusterMarkerCoordinate2;
                                                    LengthList2 = fillmissing(LengthList2,'linear');
                                                    [Idxs,~] = find(abs(diff(diff(LengthList2))) > 11);
                                                    [Idxs2,~] = find(abs(diff(LengthList2)) > 11);
                                                    if ~isempty(Idxs) && any(Idxs2 == bw_gap) && any(Idxs == bw_gap)
                                                        false_flag2 = 1;
                                                        break;
                                                    else
                                                        doubleCheckedNum2 = doubleCheckedNum2 + 1;
                                                        false_flag2 = 0;
                                                    end
                                                end
                                            end

                                            if false_flag == 0 && doubleCheckedNum >= 3 && false_flag2 == 0 && doubleCheckedNum2 >=3
                                                found = found + 1;
        ...
        %                                     disp(['          Not Found at:',cmp_marker])
                                            ...
                                            disp(['          Found using Pattern Fill Reference at: ',cmp_marker])
                                            false_flag = 2;
                                            doubleCheckedNum = 0;
                                            for ee = 1:length(currentCluster)
