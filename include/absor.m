...
% if ~verLessThan('matlab','9.1') %implicit expansion introduced
%  matmvec=@(M,v)M-v;
%  mattvec=@(M,v)M.*v;
% elseif ~verLessThan('matlab','7.4')
%  matmvec=@(M,v) bsxfun(@minus,M,v); %matrix-minus-vector
%  mattvec=@(M,v) bsxfun(@times,M,v); %matrix-times-vector
% else
%  matmvec=@matmvecHandle; %matrix-minus-vector
%  mattvec=@mattvecHandle; %matrix-times-vector
% end
matmvec=@(M,v)M-v;
mattvec=@(M,v)M.*v;
